...
export function StoryInputStep({ onNext }: StoryInputStepProps) {
  ...
  const handleStoryDescriptionChange = (value: string) => {
    updateForm({ storyDescription: value })
  }

  const handleTitleChange = (value: string) => {
    updateForm({ autoGeneratedTitle: value })
  }

  const handleEducationalThemeChange = (theme: string) => {
    updateForm({ educational_theme: theme })
  }

  const addCustomElement = () => {
    if (customElement.trim()) {
      updateForm({
        specific_elements: [...formData.specific_elements, customElement.trim()]
      })
      setCustomElement('')
    }
  }
  ...
  const removeElement = (index: number) => {
    updateForm({
      specific_elements: formData.specific_elements.filter((_, i) => i !== index)
    })
  }
